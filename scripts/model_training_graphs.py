...
with open('../scripts/runs/pose/train24/results.csv','r') as csvfile:
    ...
    for row in plots:
        ...
        y_train24.append(float(row[2].strip()))
...
with open('../scripts/runs/pose/train2/results.csv','r') as csvfile:
    ...
    for row in train2:
        ...
        y_train2.append(float(row[2].strip()))
...
with open('../scripts/runs/pose/train8/results.csv','r') as csvfile:
    ...
    for row in train8:
        ...
        y_train8.append(float(row[2].strip()))
...
with open('../scripts/runs/pose/train15/results.csv','r') as csvfile:
    ...
    for row in train15:
        ...
        y_train15.append(float(row[2].strip()))

plt.plot(x_train24, y_train24, color= "g", label = "box_loss - Train 24")
plt.plot(x_train2, y_train2, color= "b", label = "box_loss - Train 2")
plt.plot(x_train8, y_train8, color= "r", label = "box_loss - Train 8")
plt.plot(x_train15, y_train15, color= "orange", label = "box_loss - Train 15")
plt.xlabel("Epochs", labelpad=50)  # Increase distance from axis
plt.xticks(range(0, max(x_train24)+1, 2))
plt.ylabel("box_loss", labelpad=60)
plt.ylim(0.30, 2.10)
plt.yticks([i / 100 for i in range(30, 211, 15)])
